'use client'

import { forwardRef, useCallback, useEffect, useImperativeHandle, useMemo, useRef, useState } from "react"
import { motion } from "framer-motion"

interface WordObject { characters: string[]; needsSpace: boolean }

interface TextProps {
  children: React.ReactNode
  reverse?: boolean
  transition?: Record<string, unknown>
  splitBy?: "words" | "characters" | "lines" | string
  staggerDuration?: number
  staggerFrom?: "first" | "last" | "center" | "random" | number
  containerClassName?: string
  elementLevelClassName?: string
  onComplete?: () => void
  autoStart?: boolean
}

export interface VerticalCutRevealRef { startAnimation: () => void; reset: () => void }

const VerticalCutReveal = forwardRef<VerticalCutRevealRef, TextProps>(({
  children, reverse = false,
  transition = { type: "spring", stiffness: 190, damping: 22 },
  splitBy = "words", staggerDuration = 0.2, staggerFrom = "first",
  containerClassName, elementLevelClassName, onComplete, autoStart = true,
}, ref) => {
  const text = typeof children === "string" ? children : children?.toString() || ""
  const [isAnimating, setIsAnimating] = useState(false)

  const splitChars = (t: string): string[] => {
    if (typeof Intl !== "undefined" && "Segmenter" in Intl) {
      const s = new Intl.Segmenter("en", { granularity: "grapheme" })
      return Array.from(s.segment(t), ({ segment }) => segment)
    }
    return Array.from(t)
  }

  const elements = useMemo(() => {
    const words = text.split(" ")
    if (splitBy === "characters") return words.map((w, i) => ({ characters: splitChars(w), needsSpace: i !== words.length - 1 }))
    return splitBy === "words" ? text.split(" ") : splitBy === "lines" ? text.split("\n") : text.split(splitBy)
  }, [text, splitBy])

  const getDelay = useCallback((index: number) => {
    const total = splitBy === "characters"
      ? (elements as WordObject[]).reduce((a, w) => a + w.characters.length, 0)
      : elements.length
    if (staggerFrom === "first") return index * staggerDuration
    if (staggerFrom === "last") return (total - 1 - index) * staggerDuration
    if (staggerFrom === "center") return Math.abs(Math.floor(total / 2) - index) * staggerDuration
    if (staggerFrom === "random") return Math.abs(Math.floor(Math.random() * total) - index) * staggerDuration
    return Math.abs((staggerFrom as number) - index) * staggerDuration
  }, [elements, staggerFrom, staggerDuration, splitBy])

  const startAnimation = useCallback(() => { setIsAnimating(true) }, [])
  useImperativeHandle(ref, () => ({ startAnimation, reset: () => setIsAnimating(false) }))
  useEffect(() => { if (autoStart) startAnimation() }, [autoStart, startAnimation])

  const variants = {
    hidden: { y: reverse ? "-100%" : "100%" },
    visible: (i: number) => ({ y: 0, transition: { ...transition, delay: ((transition?.delay as number) || 0) + getDelay(i) } }),
  }

  const wordObjects: WordObject[] = splitBy === "characters"
    ? (elements as WordObject[])
    : (elements as string[]).map((el, i) => ({ characters: [el], needsSpace: i !== elements.length - 1 }))

  return (
    <span className={`flex flex-wrap whitespace-pre-wrap ${containerClassName || ""} ${splitBy === "lines" ? "flex-col" : ""}`}>
      <span className="sr-only">{text}</span>
      {wordObjects.map((wordObj, wi, arr) => {
        const prev = arr.slice(0, wi).reduce((s, w) => s + w.characters.length, 0)
        return (
          <span key={wi} aria-hidden className="inline-flex overflow-hidden">
            {wordObj.characters.map((ch, ci) => (
              <span key={ci} className={`whitespace-pre-wrap relative ${elementLevelClassName || ""}`}>
                <motion.span
                  custom={prev + ci}
                  initial="hidden"
                  animate={isAnimating ? "visible" : "hidden"}
                  variants={variants}
                  onAnimationComplete={wi === wordObjects.length - 1 && ci === wordObj.characters.length - 1 ? onComplete : undefined}
                  className="inline-block"
                >{ch}</motion.span>
              </span>
            ))}
            {wordObj.needsSpace && <span> </span>}
          </span>
        )
      })}
    </span>
  )
})

VerticalCutReveal.displayName = "VerticalCutReveal"
export { VerticalCutReveal }
